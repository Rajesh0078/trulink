...
const Settings = () => {
  ...
  return (
    <div className='px-5 sm:px-10 pb-20 pt-5 sm:py-4 w-full overflow-y-auto scrollbar-thumb-text-3 scrollbar-thin'>
      <div>
      ...
      <div className='flex flex-col lg:flex-row mt-4 lg:mt-6 gap-4 w-full'>
        {/* Mobile View */}
        <div className='h-8 border-b border-border lg:hidden flex gap-4 z-10 overflow-x-auto whitespace-nowrap scrollbar-none! scroll-smooth mb-1'>
          {
            SETTINGS_PREFS.flatMap(s => s.actions).map(action => {
              ...
              return (
                <button onClick={() => handleActionClick(action.key)} key={action.key}>
                  <div className={`h-full border-b ${isActive ? 'text-accent-2 border-accent-2' : 'border-transparent text-text-2'}`}>
                    {action.name}
                  </div>
                </button>
              )
          ...
          <ul className='flex flex-col'>
            {SETTINGS_PREFS.map((setting) => (
              <li key={setting.key} className='mb-4 transition-all duration-200'>
                <h2 className='text-xs font-medium ml-4 uppercase text-text-3 mb-2'>{setting.name}</h2>
